...
static func TryComp(entity : Entity, comp_type : Script) -> Component:
	for component in entity.components:
		if component.get_script() == comp_type:
			return component
	return null
...
static func RemoveComp(entity : Entity, comp_type : Script) -> void:
	...
	if (components_to_remove.size() > 1):
		push_warning("entity %s had more than one componente of type: %s!" % [str(entity.id), str(comp_type)])
...
static func EnsureComp(entity : Entity, comp_type : Script) -> void:
	for component in entity.components:
		if component.get_script() == comp_type:
			return
