...
static func all_entities_with_comp3(
	comp_type1: Script, comp_type2: Script, comp_type3: Script
) -> Array[Entity]:
	var aux: Array[Entity] = []
	var comp1_dict = ComponentRegistry.get_component_registry().get(comp_type1)
	var comp2_dict = ComponentRegistry.get_component_registry().get(comp_type2)
...
static func all_entities_with_comp4(
	comp_type1: Script, comp_type2: Script, comp_type3: Script, comp_type4: Script
) -> Array[Entity]:
	var aux: Array[Entity] = []
	var comp1_dict = ComponentRegistry.get_component_registry().get(comp_type1)
	var comp2_dict = ComponentRegistry.get_component_registry().get(comp_type2)
	...
	for entity_uid in comp1_dict.keys():
		if (
			not comp2_dict.has(entity_uid)
			or not comp3_dict.has(entity_uid)
			or not comp4_dict.has(entity_uid)
		):
			continue
