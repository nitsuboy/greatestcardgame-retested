...
func on_gdunit_event(event: GdUnitEvent) -> void:
	match event.type():
		...
		GdUnitEvent.STOP:
			...
			println_message(
				build_executed_test_case_msg(total_test_count(), total_flaky_count()),
				Color.DARK_SALMON
			)
			println_message(
		...
		GdUnitEvent.TESTSUITE_AFTER:
			if not event.reports().is_empty():
				_writer.color(Color.DARK_SALMON).style(GdUnitMessageWritter.BOLD).println_message(
					event.suite_name() + ":finalze"
				)
...
func _print_status(event: GdUnitEvent) -> void:
	if event.is_flaky() and event.is_success():
		...
		_writer.color(Color.GREEN_YELLOW).style(GdUnitMessageWritter.ITALIC).print_at(
		...
		_writer.color(Color.GOLDENROD).style(GdUnitMessageWritter.ITALIC).print_at(
		...
		_writer.color(Color.GOLDENROD).style(GdUnitMessageWritter.UNDERLINE).print_at(
...
func _print_statistics(statistics: Dictionary) -> void:
	...
	print_message(
		(
			" %d tests cases | %d errors | %d failures | %d flaky | %d skipped | %d orphans |"
			% [
