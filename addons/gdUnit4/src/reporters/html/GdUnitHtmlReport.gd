...
func add_testsuite_reports(p_resource_path: String, p_reports: Array = []) -> void:
	for report: GdUnitTestSuiteReport in _reports:
...
func add_testcase_reports(
	p_resource_path: String, p_test_name: String, p_reports: Array[GdUnitReport]
) -> void:
	for report: GdUnitTestSuiteReport in _reports:
		if report.get_resource_path() == p_resource_path:
...
func update_testsuite_counters(
	...
	for report: GdUnitTestSuiteReport in _reports:
		if report.get_resource_path() == p_resource_path:
			report.update_testsuite_counters(
				p_error_count,
				p_failure_count,
				p_orphan_count,
				p_skipped_count,
	...
	update_summary_counters(
		p_error_count, p_failure_count, p_orphan_count, p_skipped_count, p_flaky_count, 0
	)
...
func write() -> void:
	...
	GdUnitFileAccess.copy_directory(
		"res://addons/gdUnit4/src/reporters/html/template/css/", _report_path + "/css"
	)
...
func delete_history() -> int:
	return GdUnitFileAccess.delete_path_index_lower_equals_than(
		_report_path.get_base_dir(), REPORT_DIR_PREFIX, _iteration - _max_reports
	)
