	...
	if is_instance_valid(emitter):
		...
		for signal_def in emitter.get_signal_list():
			...
			if !emitter.is_connected(signal_name, _on_signal_emmited):
				var err := emitter.connect(
					signal_name, _on_signal_emmited.bind(emitter, signal_name)
				)
				if err != OK:
					push_error(
						(
							"Can't connect to signal %s on %s. Error: %s"
							% [signal_name, emitter, error_string(err)]
						)
					)
...
func _on_signal_emmited(
	...
	arg11: Variant = NO_ARG
) -> void:
	var signal_args: Array = GdArrayTools.filter_value(
		[arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, arg11], NO_ARG
	)
...
func is_signal_collecting(emitter: Object, signal_name: String) -> bool:
	@warning_ignore("unsafe_cast")
	return (
		_collected_signals.has(emitter)
		and (_collected_signals[emitter] as Dictionary).has(signal_name)
	)
