...
func save_config(path: String = CONFIG_FILE) -> GdUnitResult:
	...
	if file == null:
		...
		return GdUnitResult.error(
			"Can't write test runner configuration '%s'! %s" % [path, error_string(error)]
		)

	var to_save := {VERSION: CONFIG_VERSION, SERVER_PORT: _config.get(SERVER_PORT), TESTS: Array()}
...
func load_config(path: String = CONFIG_FILE) -> GdUnitResult:
	if not FileAccess.file_exists(path):
		return GdUnitResult.warn(
			"Can't find test runner configuration '%s'! Please select a test to run." % path
		)
	var file := FileAccess.open(path, FileAccess.READ)
	if file == null:
		var error := FileAccess.get_open_error()
		return GdUnitResult.error(
			"Can't load test runner configuration '%s'! ERROR: %s." % [path, error_string(error)]
		)
	...
	if not content.is_empty() and content[0] == "{":
		...
		if error != OK:
			return (
				...
				. error(
					(
						"The runner configuration '%s' is invalid! The format is changed please delete it manually and start a new test run."
						% path
					)
				)
			)
...
func convert_test_json_to_test_cases(jsons: Array) -> Array[GdUnitTestCase]:
	if jsons.is_empty():
		return []
	var tests := jsons.map(
		func(d: Dictionary) -> GdUnitTestCase:
