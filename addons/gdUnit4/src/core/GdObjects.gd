...
enum COMPARE_MODE { OBJECT_REFERENCE, PARAMETER_DEEP_TEST }
...
static func obj2dict(obj: Object, hashed_objects := Dictionary()) -> Dictionary:
	...
	for property in obj.get_property_list():
		...
		if property_value is GDScript or property_value is Callable or property_value is RegEx:
			continue
		if (
			property["usage"] & PROPERTY_USAGE_SCRIPT_VARIABLE | PROPERTY_USAGE_DEFAULT
	...
	if obj is Node:
		...
		dict["childrens"] = childrens.map(
			func(child: Object) -> Dictionary: return obj2dict(child, hashed_objects)
		)
	if obj is TreeItem:
...
static func equals(
	...
	compare_mode: COMPARE_MODE = COMPARE_MODE.PARAMETER_DEEP_TEST
) -> bool:
	return _equals(obj_a, obj_b, case_sensitive, compare_mode, [], 0)


static func equals_sorted(
	obj_a: Array[Variant],
	obj_b: Array[Variant],
	case_sensitive: bool = false,
	compare_mode: COMPARE_MODE = COMPARE_MODE.PARAMETER_DEEP_TEST
	...
	match type_a:
		...
		TYPE_DICTIONARY:
			...
			for key: Variant in obj_a.keys():
				...
				if not _equals(
					value_a, value_b, case_sensitive, compare_mode, deep_stack, stack_depth
				):
					return false
			return true
			...
			if ClassDB.class_exists(clazz_name):
				if Engine.has_singleton(clazz_name):
					return GdUnitResult.error(
						"Not allowed to create a instance for singelton '%s'." % clazz_name
					)
...
static func extract_class_name(clazz: Variant) -> GdUnitResult:
	...
	if is_primitive_type(clazz):
		return GdUnitResult.error(
			"Can't extract class name for an primitive '%s'" % type_as_string(typeof(clazz))
		)
...
static func extract_inner_clazz_names(
	clazz_name: String, script_path: PackedStringArray
) -> PackedStringArray:
...
static func default_value_by_type(type: int) -> Variant:
	...
	push_error(
		(
			"Can't determine a default value for type: '%s', Please create a Bug issue and attach the stacktrace please."
			% type
		)
	)
	return null
