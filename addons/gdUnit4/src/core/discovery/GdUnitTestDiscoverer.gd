...
static func run() -> Array[GdUnitTestCase]:
	...
	t.start(
		func() -> Array[GdUnitTestCase]:
			...
			var test_suite_directories: PackedStringArray = (
				GdUnitCommandHandler.scan_all_test_directories(GdUnitSettings.test_root_folder())
			)
			...
			if !recovered_tests.is_empty():
				console_log(
					(
						"Recovery last test session successfully, %d tests restored."
						% recovered_tests.size()
					),
					true
				)
...
static func restore_last_session() -> void:
	...
	t.start(
		func() -> void:
			...
			GdUnitSignals.instance().gdunit_event.emit(GdUnitEventTestDiscoverEnd.new(0, 0))
			console_log(
				"Recovery last test session successfully, %d tests restored." % test_cases.size(),
				true
			)
	)
	t.wait_to_finish()
...
static func recover_test_guid(
	current: GdUnitTestCase, recovered_tests: Array[GdUnitTestCase]
) -> void:
...
static func console_log_discover_results(tests: Array[GdUnitTestCase]) -> void:
	var grouped_by_suites := GdArrayTools.group_by(
		tests, func(test: GdUnitTestCase) -> String: return test.source_file
	)
	for suite_tests: Array in grouped_by_suites.values():
		var test_case: GdUnitTestCase = suite_tests[0]
		console_log(
			(
				"Discover: TestSuite %s with %d tests fount"
				% [test_case.source_file, suite_tests.size()]
			)
		)
	console_log(
		(
			"Discover tests done, %d TestSuites and total %d Tests found. "
			% [grouped_by_suites.size(), tests.size()]
		)
	)
	console_log("")
...
static func discover_tests(source_script: Script, discover_sink := default_discover_sink) -> void:
	if source_script is GDScript:
		var test_names := source_script.get_script_method_list().filter(filter_tests).map(
			func(method: Dictionary) -> String: return method["name"]
		)
		# no tests discovered?
