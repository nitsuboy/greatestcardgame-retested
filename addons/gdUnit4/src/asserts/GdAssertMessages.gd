...
static func input_event_as_text(event: InputEvent) -> String:
	...
	if event is InputEventKey:
		...
		text += (
			"InputEventKey : key='%s', pressed=%s, keycode=%d, physical_keycode=%s"
	...
	if event is InputEventWithModifiers:
		...
		text += (
			", shift=%s, alt=%s, control=%s, meta=%s, command=%s"
			% [
...
static func orphan_detected_on_test(count: int) -> String:
	return "%s\n Detected <%d> orphan nodes during test execution!" % [_warning("WARNING:"), count]
